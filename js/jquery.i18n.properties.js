...
(function ($) {
    ...
    $.i18n.properties = function (settings) {
        ...
        files.forEach(function (file) {
            ...
            shortFileName = settings.path + file + '/lang/' + shortCode + '/';
            // 3. with language code and country code (eg, Messages_pt_BR.properties)
            if (settings.language.length >= 5) {
                var longCode = settings.language.substring(0, 5);
                longFileName = settings.path + file + '/lang/' + longCode + '/';
        ...
        if (settings.debug) {
  	    ...
  	    if (filename !== null && typeof filename !== 'undefined') {
            ...
            if(sessionStorage.getItem('langData')){
              var langData = JSON.parse(sessionStorage.getItem('langData'))
              if(langData.file==filename){
                parseData(langData.content, settings);
                nextFile();
                return false;
              }
            }
            $.ajax({
                ...
                success: function (data, status) {
                    ...
                    sessionStorage.setItem('langData',JSON.stringify({file:filename,content:data}))
